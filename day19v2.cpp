...
vec<int> getDiffs(const vec<Point3D>& beacons) {
    vec<int> diffs;
    diffs.reserve(64);
    for (Point3D b1 : beacons)
        for (Point3D b2 : beacons)
            if (b1 < b2) {
                diffs.push_back(
                    std::abs(b1.x - b2.x)
                    + (std::abs(b1.y - b2.y))
...
void adjustScanner(Scanner& scanner, const Scanner& known,
                   int perm[3], int neg, Point3D offset) {
    ...
    cout << "Found scanner " << scanner.id
         << " intersecting with " << known.id << "\n"
         << scanner.position << "\n";
}

int setIntersectionSize(const auto& vec1, const auto& vec2) {
    Counter c;
    stdr::set_intersection(vec1, vec2, std::back_inserter(c));
    return c.count;
}

bool tryMatchingScanners(const Scanner& sknown, Scanner& sUnknown) {
    if (setIntersectionSize(sknown.diffs, sUnknown.diffs) < 11 * 12 / 2)
        return false;
        ...
        do {
            ...
            for (const auto& p2 : unknown_beacons) {
                ...
                if (stdr::count_if(unknown_beacons_copy,
                    [&](const Point3D& p) { return known_set.contains(p); }) >= 12) {
...
int main() {
    ...
    for (const auto& s2 : scanners) {
        ...
        max_distance = std::max(max_distance,
            std::abs(s1.position.x - s2.position.x) + std::abs(s1.position.y - s2.position.y) + std::abs(s1.position.z - s2.position.z));
    }
